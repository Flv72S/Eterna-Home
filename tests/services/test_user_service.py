...
# Test Create User
def test_create_user_success(session: Session, user_create: UserCreate):
...
def test_create_user_duplicate_email_raises_error(session: Session, user_create: UserCreate):
...
def test_create_user_hashes_password(session: Session, user_create: UserCreate):
...
# Test Get User
def test_get_user_by_id_success(session: Session, user_create: UserCreate):
    """Test che verifica il recupero corretto di un utente tramite ID."""
    created_user = UserService.create_user(session, user_create)
    retrieved_user = UserService.get_user(session, created_user.id)
...
def test_get_user_by_id_not_found(session: Session):
    ...
    non_existent_id = 999
    user = UserService.get_user(session, non_existent_id)
    
    assert user is None

# Test Get User by Email
def test_get_user_by_email_success(session: Session, user_create: UserCreate):
    """Test che verifica il recupero corretto di un utente tramite email."""
    created_user = UserService.create_user(session, user_create)
    retrieved_user = UserService.get_user_by_email(session, user_create.email)
...
def test_get_user_by_email_not_found(session: Session):
    ...
    user = UserService.get_user_by_email(session, non_existent_email)
    
    assert user is None

# Test Update User
def test_update_user_success(session: Session, user_create: UserCreate):
    ...
    updated_user = UserService.update_user(session, created_user.id, update_data)
    
    assert updated_user.email == update_data.email
    assert updated_user.is_active == update_data.is_active
    assert updated_user.id == created_user.id

def test_update_user_password_is_hashed(session: Session, user_create: UserCreate):
...
def test_update_user_not_found(session: Session):
    ...
    with pytest.raises(HTTPException) as exc_info:
        UserService.update_user(session, non_existent_id, update_data)
    
    assert exc_info.value.status_code == 404
    assert "Utente non trovato" in str(exc_info.value.detail)

# Test Delete User
def test_delete_user_success(session: Session, user_create: UserCreate):
    """Test che verifica l'eliminazione corretta di un utente."""
    created_user = UserService.create_user(session, user_create)
    
    result = UserService.delete_user(session, created_user.id)
    assert result is True
    
    # Verifica che l'utente sia stato effettivamente eliminato
    deleted_user = UserService.get_user(session, created_user.id)
