...
def test_login_with_valid_credentials(client: TestClient, test_user_shared_session):
...
def test_login_with_disabled_user(client: TestClient, test_user_shared_session):
    """Test login con utente disabilitato."""
    # Disabilita l'utente
    with next(get_db()) as session:
        user = session.exec(
            Session.select(User).where(User.email == "test@example.com")
        ).first()
...
def test_debug_user_exists_in_database(client: TestClient, test_user_shared_session: User, engine):
    ...
    with Session(engine) as session:
        # Cerca per email
        user_by_email = session.exec(select(User).where(User.email == test_user_shared_session.email)).first()
        ...
        # Cerca per username
        user_by_username = session.exec(select(User).where(User.username == test_user_shared_session.username)).first()
...
def test_login_without_username(client: TestClient):
...
def test_login_without_password(client: TestClient):
...
def test_login_with_empty_credentials(client: TestClient):
