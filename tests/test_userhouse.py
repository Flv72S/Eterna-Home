...
def test_user_can_access_multiple_houses(db_session, test_user, house1, house2, house3):
    ...
    roles = [uh.role for uh in user_houses]
...
def test_user_house_roles(db_session, test_user, house1, house2):
    ...
    # Verify roles
    assert owner_rel.role == "owner"
    assert resident_rel.role == "resident"
...
def test_house_role_upgrade(db_session, test_user, house1):
    ...
    assert relationship.role == "guest"
    
    # Upgrade to resident
    relationship.role = "resident"
    db_session.commit()
    ...
    assert updated_relationship.role == "resident"
...
def test_house_access_validation(db_session, test_user, house1):
    ...
    has_access = db_session.query(UserHouse).filter(
        UserHouse.user_id == test_user.id,
        UserHouse.house_id == house1.id
    ).first() is not None
    ...
    non_existent_house_id = 99999
    no_access = db_session.query(UserHouse).filter(
        UserHouse.user_id == test_user.id,
        UserHouse.house_id == non_existent_house_id
    ).first() is None
