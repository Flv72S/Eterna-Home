...
TENANT_2_UUID = "c4c3d2e1-2345-6789-abcd-ef1234567890"
...
def test_multitenant_api_isolation(tenant1_user, tenant2_user):
    ...
    token1 = create_access_token(data={"sub": tenant1_user.email, "tenant_id": TENANT_1_UUID})
    token2 = create_access_token(data={"sub": tenant2_user.email, "tenant_id": TENANT_2_UUID})
    ...
    assert TENANT_1_UUID in token1
    assert TENANT_2_UUID in token2
    assert TENANT_1_UUID not in token2
    assert TENANT_2_UUID not in token1
