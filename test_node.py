...
TEST_NODE_NAME = "Nodo Test"
TEST_NODE_TYPE = "sensor"  # Deve corrispondere a NodeType.SENSOR
TEST_NODE_STATUS = "active"  # Deve corrispondere a NodeStatus.ACTIVE
TEST_NODE_LOCATION = {
    "x": 10.5,
    "y": 20.3,
    "z": 5.0
...
def test_create_house(access_token: str) -> tuple[TestResult, Optional[int]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not house_id:
                return TestResult("Creazione Casa", False, "ID casa non presente nella risposta"), None
            ...
            if response_data.get("name") != TEST_HOUSE_NAME or response_data.get("address") != TEST_HOUSE_ADDRESS:
                return TestResult("Creazione Casa", False, "Dati casa non corrispondenti"), None
            
            return TestResult("Creazione Casa", True, f"Casa creata con ID: {house_id}"), house_id
        else:
            return TestResult("Creazione Casa", False, f"Codice di stato inatteso: {response.status_code}"), None
    except Exception as e:
        logger.error(f"Errore durante la creazione della casa: {str(e)}", exc_info=True)
        return TestResult("Creazione Casa", False, str(e)), None

def test_create_node(access_token: str, house_id: int) -> TestResult:
    """Test di creazione nodo"""
    try:
        logger.info("\nEsecuzione test creazione nodo...")
        logger.info(f"URL: {BASE_URL}/nodes/")
        logger.info(f"Dati inviati: name={TEST_NODE_NAME}, type={TEST_NODE_TYPE}, status={TEST_NODE_STATUS}")
        ...
        response = requests.post(
            ...
            json={
                ...
                "house_id": house_id,
                "location_x": TEST_NODE_LOCATION["x"],
                "location_y": TEST_NODE_LOCATION["y"],
                "location_z": TEST_NODE_LOCATION["z"]
            },
            headers=get_headers(access_token)
        )
        ...
        if response.status_code == 200:
            ...
            # Verifica che tutti i campi restituiti corrispondano a quelli inviati
            if (response_data.get("name") != TEST_NODE_NAME or
                response_data.get("type") != TEST_NODE_TYPE or
                response_data.get("status") != TEST_NODE_STATUS or
                response_data.get("house_id") != house_id or
                response_data.get("location_x") != TEST_NODE_LOCATION["x"] or
                response_data.get("location_y") != TEST_NODE_LOCATION["y"] or
                response_data.get("location_z") != TEST_NODE_LOCATION["z"]):
                return TestResult("Creazione Nodo", False, "Dati nodo non corrispondenti")
            
            return TestResult("Creazione Nodo", True, f"Nodo creato con ID: {node_id}")
        else:
            return TestResult("Creazione Nodo", False, f"Codice di stato inatteso: {response.status_code}")
    except Exception as e:
        logger.error(f"Errore durante la creazione del nodo: {str(e)}", exc_info=True)
        return TestResult("Creazione Nodo", False, str(e))

def main():
    """Funzione principale di esecuzione dei test"""
    logger.info("Avvio Test Creazione Nodo...")
    logger.info("=" * 50)
    ...
    house_result, house_id = test_create_house(access_token)
    ...
    # Test di creazione nodo
    node_result = test_create_node(access_token, house_id)
    logger.info(f"\nTest Creazione Nodo: {'SUCCESSO' if node_result.success else 'FALLITO'}")
    logger.info(f"Messaggio: {node_result.message}")
